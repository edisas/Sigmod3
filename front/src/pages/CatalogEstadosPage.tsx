...
export default function CatalogEstadosPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE}/catalogos/estados/listado?${params.toString()}`, {
      ...
      setItems([]);
      setTotal(0);
      setError('No fue posible cargar el catálogo de estados.');
    } finally {
      setIsLoading(false);
    }
  ...
  const inactivate = async (id: number) => {
    if (!token) return;
    if (!window.confirm('¿Deseas inactivar este estado?')) return;
    const response = await fetch(`${API_BASE}/catalogos/estados/${id}`, {
      method: 'DELETE',
      headers: { Authorization: `Bearer ${token}` },
    });
    if (!response.ok) {
      setError('No se pudo inactivar el estado.');
      return;
    }
    await load();
  };
  ...
  return (
    <div className="p-4 md:p-8 space-y-6">
      <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-3">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 dark:text-white">Catálogo de Estados</h1>
        ...
        <button
          ...
          onClick={() => navigate('/catalogos/estados/nuevo')}
        >
          <Icon name="add" className="text-base" /> Nuevo Estado
        </button>
      </div>
      ...
      <div className="rounded-xl border border-slate-200 bg-white p-4 grid grid-cols-1 md:grid-cols-5 gap-3">
        <input
          ...
          placeholder="Buscar por clave, nombre o abreviatura"
