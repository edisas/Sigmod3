...
interface MunicipioListResponse {
  items: MunicipioItem[];
  total: number;
  page: number;
  page_size: number;
...
export default function CatalogMunicipiosPage() {
  ...
  const load = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE}/catalogos/municipios/listado?${params.toString()}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      if (!response.ok) throw new Error(`HTTP ${response.status}`);
      const data = (await response.json()) as MunicipioListResponse;
      setItems(data.items);
      setTotal(data.total);
    } catch {
      setItems([]);
      setTotal(0);
      setError('No fue posible cargar el catálogo de municipios.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  useEffect(() => {
    void load();
  }, [token, page, pageSize, search, estadoFilter, estatusFilter]);

  const inactivate = async (id: number) => {
    if (!token) return;
    if (!window.confirm('¿Deseas inactivar este municipio?')) return;
    const response = await fetch(`${API_BASE}/catalogos/municipios/${id}`, {
      method: 'DELETE',
      headers: { Authorization: `Bearer ${token}` },
    });
    if (!response.ok) {
      setError('No se pudo inactivar el municipio.');
      return;
    }
    await load();
  };
  ...
  const exportCurrent = () => {
    const rows = items.map((i) => [i.id, i.estado_nombre ?? i.estado_id, i.clave, i.nombre, i.clave_geo, i.estatus_id === 1 ? 'Activo' : 'Inactivo']);
    downloadCsv('catalogo_municipios_vista.csv', ['ID', 'Estado', 'Clave', 'Nombre', 'Clave GEO', 'Estatus'], rows);
    setExportOpen(false);
  };
  ...
  const exportAll = async () => {
    ...
    const data = (await response.json()) as MunicipioListResponse;
    const rows = data.items.map((i) => [i.id, i.estado_nombre ?? i.estado_id, i.clave, i.nombre, i.clave_geo, i.estatus_id === 1 ? 'Activo' : 'Inactivo']);
    downloadCsv('catalogo_municipios_todo.csv', ['ID', 'Estado', 'Clave', 'Nombre', 'Clave GEO', 'Estatus'], rows);
    setExportOpen(false);
  };
  ...
  return (
    <div className="p-4 md:p-8 space-y-6">
      <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-3">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 dark:text-white">Catálogo de Municipios</h1>
          <p className="text-sm text-slate-600 dark:text-slate-300 mt-1">Listado con filtros por estado y estatus.</p>
        </div>
        <button className="inline-flex items-center gap-2 rounded-xl bg-primary text-white px-4 py-2" onClick={() => navigate('/catalogos/municipios/nuevo')}>
          Nuevo Municipio
        </button>
      </div>

      <div className="rounded-xl border border-slate-200 bg-white p-4 grid grid-cols-1 md:grid-cols-6 gap-3">
        <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Buscar por nombre, clave, estado, clave geo" className="rounded-lg border border-slate-300 px-3 py-2 text-sm md:col-span-2" />
        <select className="rounded-lg border border-slate-300 px-3 py-2 text-sm" value={estadoFilter} onChange={(e) => { setEstadoFilter(e.target.value ? Number(e.target.value) : ''); setPage(1); }}>
        ...
        <div className="flex gap-2">
          ...
          <button className="rounded-lg border border-slate-300 px-3 py-2 text-sm" onClick={() => { setQ(''); setSearch(''); setEstadoFilter(''); setEstatusFilter(1); setPage(1); }}>Limpiar</button>
        </div>
            ...
            {isLoading ? (
              ...
              items.map((item) => (
                <tr key={item.id} className="border-t border-slate-100">
                  ...
                  <td className="px-4 py-3">{item.clave}</td>
                  <td className="px-4 py-3">{item.nombre}</td>
                  <td className="px-4 py-3">{item.clave_geo}</td>
                  <td className="px-4 py-3">{item.estatus_id === 1 ? 'Activo' : 'Inactivo'}</td>
                  <td className="px-4 py-3 text-right">
                    <div className="inline-flex gap-2">
                      <button className="rounded-md border border-primary px-2 py-1 text-primary" onClick={() => navigate(`/catalogos/municipios/${item.id}/editar`)}>Editar</button>
